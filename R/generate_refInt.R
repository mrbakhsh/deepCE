  ...
  #' @param data A data frame with interacting proteins in the first two
  #' columns.
  ...
  #' data input. Protein pairs involve in same protein complexes are assigned
  #' to 1, otherwise 0.
  ...
  generate_refInt <- function(data,
    ...
    col1 <- node_columns[1]
    col2 <- node_columns[2]
    prot_1 <- as.character(data[[col1]])
    prot_2 <- as.character(data[[col2]])
    ...
    labels[class.lab_index] <- adj_m[idxing_dat]

    data_labeled <- cbind(data, labels)
